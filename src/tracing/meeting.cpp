...
void *append_meetings_unit(void *arg){
...
void workbench::append_meetings(){
	...
	for(int i=0;i<tctx.config->num_threads;i++){
		pthread_create(&threads[i], NULL, append_meetings_unit, (void *)&tctx);
	}
	for(int i = 0; i < tctx.config->num_threads; i++ ){
		void *status;
	...
	logt("append meeting: %d meetings active",start,mc);
}
...
void *update_meetings_unit(void *arg){
	...
	while(ctx->next_batch(start,end)){
		for(uint rid=start;rid<end;rid++){
			...
			meeting_unit *bucket = bench->meeting_buckets+bid*bench->meeting_bucket_capacity;
			bool updated = false;
			for(uint i=0;i<bench->meeting_buckets_counter[bid];i++){
			...
			bench->reaches[rid].updated = updated;
		}
	}
	return NULL;
}
