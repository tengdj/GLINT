...
void *sort_meetings_unit(void *arg){
	...
	while(ctx->next_batch(start,end)){
		for(uint bid=start;bid<end;bid++){
			...
			uint size_new = min(bench->meeting_buckets_counter[bench->current_bucket][bid],bench->meeting_bucket_capacity);
			meeting_unit tmp;
			if(size_new>=2){
				for(uint i=0;i<size_new-1;i++){
					for(uint j=0;j<size_new-i-1;j++){
						if(bucket_new[j].pid1<bucket_new[j+1].pid1){
							// swap
							tmp = bucket_new[j];
							bucket_new[j] = bucket_new[j+1];
							bucket_new[j+1] = tmp;
						}
					}
				}
...
void workbench::update_meetings(){
	...
	for(int i=0;i<tctx.config->num_threads;i++){
		pthread_create(&threads[i], NULL, sort_meetings_unit, (void *)&tctx);
	}
	for(int i = 0; i < tctx.config->num_threads; i++ ){
		void *status;
		pthread_join(threads[i], &status);
	}
	logt("sort new meeting",start);
